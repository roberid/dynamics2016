...
are somewhat less dependent on environment \citep{haines2006, bamford2009}.  For
...
would directly impact morphology, therefore in order to explain the
...
transformations is perhaps required.  Another important question is what are
...
``pre-processing'')?  Simulations have shown that between $\sim 25$
and $45$ per cent of infalling cluster galaxies may have been
pre-processed \citep{mcgee2009, delucia2012}, whereas \citet{hou2014}
find observationally $\sim 25$ per cent of infalling galaxies are
within subhaloes for their highest mass clusters.
\par
...
the impact of group dynamics is less clear \citep{hou2013,
  ribeiro2013} (REF) which is likely suggestive of a more modest role.
Therefore,
...
compared to galaxies falling into haloes (and this excess is interestingly
absent in the lowest mass bin).  When looking at disc fraction, this
...
plot the field excess, in star forming fraction (stars) and disc
fraction (circles), versus stellar mass for both groups (blue) and
...
groups would lead to a more minute difference between galaxies infalling into
...
considering star forming
disc fractions for galaxies within the virialized region of the halo a
subtle dependence 
on group dynamics emerges.  In particular, a marginal marching order is
present, galaxies in G groups have the lowest star forming and
...
difference is observed between the G and NG samples.  As was so for the
infalling region, we see qualitatively similar trends in both star forming and
disc fractions.
...
\citet{ribeiro2013} propose using the Hellinger distance measure to
discriminate between G and NG groups using a catalogue of SDSS group galaxies
...
mechanisms within the group environment for longer.
...
In this paper we have investigated the dependence of galaxy properties
(namely, star forming and disc fractions) on host group dynamics.  To
do so we construct a carefully matched sample of galaxies housed by
Gaussian groups, galaxies housed by non-Gaussian groups, as well as
...
\begin{enumerate}
  \item Star forming and disc fractions for galaxies in the infalling
    region of their host halo show no systematic dependence on group
    dynamics.

  \item We observe only a small difference between the star forming
    and disc fractions of field galaxies compared to those infalling
    into a group halo, however this difference is larger when
    considering only galaxies falling into clusters likely as a result
    of pre-processing.
