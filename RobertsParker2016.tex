...
are far less independent of environment \textbf{(REF)}.  For
high-mass galaxies quenching is likely driven by internal, secular
processes such as feedback and winds from AGN or supernovae
\textbf{(REF)}.  This dichotomy between high and low mass galaxies is
...
Galaxy harassment \citep[e.g.][]{moore1996}, mergers \textbf{(REF)},
...
morphology.  Starvation and/or ram-pressure stripping are often favoured
as quenching mechanisms \textbf{(REF)} but it is unlikely that either
would directly impact morphology, therefore in order to explain the
similarities between observed star forming and disc fraction trends
\citep{roberts2016} \textbf{(REF)} it
seems that an additional process to efficiently drive morphological
transformations is required.  Another important question is what are
...
\par
In this paper we further investigate the star forming and
morphological properties of galaxies infalling into haloes versus
galaxies virialized at the group centre.  In particular study how
these properties depend on the dynamical state of the host halo, to
elucidate whether dependences on dynamical state are in place during
